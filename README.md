...
раза в час** (`POLL_INTERVAL_MINUTES`, по умолчанию 60). Запуск вручную:
...
- **Отклик Telegram и задержки.** В режиме long-poll (`LOOP_SECONDS=270`, cron `*/5`)
  ответы на команды/кнопки приходят почти мгновенно, пока идёт запуск. Между запусками
  бывает короткий разрыв (cron в GitHub не идеально точен и может задерживаться), но
  сообщения Telegram не теряются — хранятся до 24 ч и обработаются следующим запуском.
  Запуск раз в минуту через cron в GitHub Actions **невозможен** — минимум `*/5`.

- **Поиск раз в час.** Сам опрос Bazaraki ограничен `POLL_INTERVAL_MINUTES` (по умолч.
  60) независимо от частоты запусков — чтобы экономить кредиты scraping-сервиса.
